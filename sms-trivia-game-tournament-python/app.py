...
def generate_question(category="general"):
    resp = requests.post(INFERENCE_URL, headers={"Authorization": f"Bearer {TELNYX_API_KEY}", "Content-Type": "application/json"},
        json={"model": AI_MODEL, "messages": [{"role": "system", "content": f"Generate a {category} trivia question. Return JSON: question (string, timeout=10), options (list of 4 strings), correct_index (0-3), fun_fact (string, 1 sentence)."}],
