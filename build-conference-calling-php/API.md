...
Retrieve data from `/conferences/{conferenceId}`.
...
## `POST /conferences/{conferenceId}/end`

Submit data to `/conferences/{conferenceId}/end`.
...
```bash
curl -X POST http://localhost:5000/conferences/{conferenceId}/end \
  -H "Content-Type: application/json" \
  -d '{}'
```
