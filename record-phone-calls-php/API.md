...
Submit data to `/calls/recording/start`.
...
```bash
curl -X POST http://localhost:5000/calls/recording/start \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
Submit data to `/calls/recording/stop`.
...
```bash
curl -X POST http://localhost:5000/calls/recording/stop \
  -H "Content-Type: application/json" \
  -d '{}'
```
